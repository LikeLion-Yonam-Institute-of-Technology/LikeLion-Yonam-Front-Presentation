...
const CodePreview = ({ 
  ...
  // children이 있고 아직 편집 안 했으면 children, 그 외에는 항상 iframe
  ...
  return (
    <div className="code-preview-container animate-fade-in" style={{ marginTop: '1.5rem', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', borderRadius: '12px', overflow: 'hidden', border: '1px solid #eee' }}>
      ...
      <div className="code-preview-header" style={{ background: '#f8f9fa', padding: '12px 20px', display: 'flex', alignItems: 'center', borderBottom: '1px solid #eee' }}>
        ...
        {hasEdited && (
          <span style={{ marginLeft: 'auto', fontSize: '0.7rem', color: '#10B981', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '4px' }}>
            ● LIVE
          </span>
        )}
      </div>
      ...
      <div className="code-preview-body" style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', background: '#fff', minHeight: height }}>
        ...
        <div className="code-section" style={{ borderRight: '1px solid #f0f0f0', background: '#fafafa', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '8px 20px', fontSize: '0.7rem', color: '#999', fontWeight: 'bold', borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between' }}>
